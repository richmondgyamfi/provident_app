...
@section('content')
<form id="loan-form" method="POST" action="{{ route('loans.store') }}" enctype="multipart/form-data" class="p-4 sm:p-8 space-y-6 sm:space-y-8">
    @csrf
        ...
        <div id="step-panel-1"
            ...
            <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                <h2 class="text-slate-900 dark:text-slate-100 text-xl sm:text-[22px] font-bold leading-tight tracking-[-0.015em] mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select id="loan-type"
                            ...
                            <option value="facility">Facility Loan</option>
                            <option value="emergency">Emergency Loan</option>
                        </select>
                    </div>
                    <!-- Term -->
                    <div class="flex flex-col gap-2">
                        <label class="text-slate-900 dark:text-slate-100 text-sm font-bold">Repayment Period (Months)</label>
                        <input id="loan-term"
            ...
            <div class="bg-primary/5 p-5 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 border-b border-slate-200 dark:border-slate-800">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 dark:text-slate-400 text-sm">Estimated figures based on current interest rates.</p>
                    </div>
                </div>
                <div class="flex gap-6 sm:gap-8 w-full sm:w-auto">
                    <div class="text-left sm:text-right">
                        <p class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Interest Rate</p>
                    ...
                    <div class="text-left sm:text-right border-l border-slate-200 dark:border-slate-800 pl-6 sm:pl-8">
                        <p class="text-xs text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Monthly Pay</p>
            ...
            <div class="p-5 sm:p-8 flex flex-wrap justify-between items-center gap-4 bg-slate-50 dark:bg-slate-900/50">
                <button onclick="goToStep(null)" class="flex items-center justify-center px-6 h-12 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                    Cancel
                </button>
                <button onclick="goToStep(2)" class="flex items-center justify-center px-6 sm:px-8 h-12 rounded-lg bg-primary text-white font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
                    Save &amp; Next
        ...
        <div id="step-panel-2" class="hidden space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                    <h2 class="text-slate-900 dark:text-slate-100 text-xl sm:text-[22px] font-bold mb-1">Loan Calculation Breakdown</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Review the full repayment schedule before proceeding.</p>
                </div>

                <!-- Summary Cards -->
                <div class="p-5 sm:p-8 grid grid-cols-2 md:grid-cols-4 gap-4 border-b border-slate-200 dark:border-slate-800">
                ...
                <!-- Extra details row -->
                <div class="px-5 sm:px-8 py-5 grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
                ...
                <div class="p-5 sm:p-8">
                        ...
                        <table class="w-full text-left text-sm">
                            <thead>
                                <tr class="bg-slate-50 dark:bg-slate-800/50">
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Month</th>
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Opening Balance</th>
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Principal</th>
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Interest</th>
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Monthly Payment</th>
                                    <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Closing Balance</th>
                                </tr>
                            </thead>
                    ...
                    <p class="text-xs text-slate-400 mt-3 italic">* Figures shown are estimates. Final amounts are subject to fund approval and disbursement date.</p>
                </div>

                <div class="p-5 sm:p-8 flex flex-wrap justify-between items-center gap-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800">
                    <button onclick="goToStep(1)" class="flex items-center justify-center px-6 h-12 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                        <span class="material-symbols-outlined mr-2 text-[20px]">arrow_back</span>Back
                    </button>
                    <button onclick="goToStep(3)" class="flex items-center justify-center px-6 sm:px-8 h-12 rounded-lg bg-primary text-white font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
                        Save &amp; Next
                        <span class="material-symbols-outlined ml-2 text-[20px]">arrow_forward</span>
                    </button>
                </div>
        ...
        <div id="step-panel-3" class="hidden space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                    <h2 class="text-slate-900 dark:text-slate-100 text-xl sm:text-[22px] font-bold mb-1">Supporting Documents</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Upload the required documents to support your loan application. Accepted formats: PDF, JPG, PNG (max 5MB each).</p>
                </div>

                <div class="p-5 sm:p-8 grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-slate-200 dark:border-slate-800">

                    <!-- Required document uploads -->
                    @php
                        $docs = [
                            ['id' => 'doc-id',      'label' => 'National ID / Passport',       'desc' => 'A valid government-issued photo ID.',            'icon' => 'badge',         'required' => true],
                            ['id' => 'doc-payslip', 'label' => 'Latest 3 Pay Slips',            'desc' => 'Signed payslips from the last three months.',    'icon' => 'receipt_long',  'required' => true],
                            ['id' => 'doc-letter',  'label' => 'Employment / Confirmation Letter', 'desc' => 'Letter from your employer confirming employment.', 'icon' => 'description', 'required' => true],
                            ['id' => 'doc-bank',    'label' => 'Bank Statement (3 months)',     'desc' => 'Official statement from your bank.',            'icon' => 'account_balance','required' => true],
                            ['id' => 'doc-purpose', 'label' => 'Proof of Purpose (if applicable)', 'desc' => 'Quotation, invoice, or supporting evidence.', 'icon' => 'attach_file',   'required' => false],
                            ['id' => 'doc-other',   'label' => 'Any Other Supporting Document', 'desc' => 'Optional additional supporting files.',         'icon' => 'folder_open',   'required' => false],
                        ];
                    @endphp
                    ...
                    @foreach ($docs as $doc)
                        <div class="flex flex-col gap-2">
                            <div class="flex items-center gap-2">
                                <label for="{{ $doc['id'] }}" class="text-slate-900 dark:text-slate-100 text-sm font-bold">
                                    {{ $doc['label'] }}
                                </label>
                                @if($doc['required'])
                                    <span class="text-[10px] font-bold text-white bg-accent-red px-1.5 py-0.5 rounded">Required</span>
                                @else
                                    <span class="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">Optional</span>
                                @endif
                            </div>
                            <label for="{{ $doc['id'] }}"
                                ...
                                <div class="size-10 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-primary group-hover:bg-primary/10 transition-colors shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p id="{{ $doc['id'] }}-name" class="text-xs font-semibold text-primary mt-0.5 hidden"></p>
                                </div>
                                <span class="material-symbols-outlined text-slate-300 group-hover:text-primary text-[20px] shrink-0">upload</span>
                                <input type="file" id="{{ $doc['id'] }}" class="hidden" accept=".pdf,.jpg,.jpeg,.png"
                ...
                <div class="p-5 sm:p-8 bg-slate-50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach ([
                            ...
                            'Scanned copies must be in colour.',
                            'Bank statements must show your name and account number.',
                            'Documents must be dated within the last 3 months where applicable.',
                        ] as $tip)
                ...
                <div class="p-5 sm:p-8 flex flex-wrap justify-between items-center gap-4 bg-slate-50 dark:bg-slate-900/50">
                    <button onclick="goToStep(2)" class="flex items-center justify-center px-6 h-12 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                        <span class="material-symbols-outlined mr-2 text-[20px]">arrow_back</span>Back
                    </button>
                    <button onclick="goToStep(4)" class="flex items-center justify-center px-6 sm:px-8 h-12 rounded-lg bg-primary text-white font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
                        Save &amp; Next
                        <span class="material-symbols-outlined ml-2 text-[20px]">arrow_forward</span>
                    </button>
                </div>
        ...
        <div id="step-panel-4" class="hidden space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                    <h2 class="text-slate-900 dark:text-slate-100 text-xl sm:text-[22px] font-bold mb-1">Terms &amp; Conditions</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Please read and accept the terms below before submitting your application.</p>
                </div>
                ...
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="flex flex-col gap-0.5">
                            ...
                            <p id="terms-principal" class="text-base font-black text-slate-900 dark:text-slate-100">₵0.00</p>
                        </div>
                        <div class="flex flex-col gap-0.5">
                        ...
                        <div class="flex flex-col gap-0.5">
                            ...
                            <p id="terms-rate" class="text-base font-black text-accent-red">14% p.a.</p>
                        </div>
                        <div class="flex flex-col gap-0.5">
                            <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold">Monthly Payment</p>
                            <p id="terms-monthly" class="text-base font-black text-slate-900 dark:text-slate-100">₵0.00</p>
                        </div>
                        <div class="flex flex-col gap-0.5">
                            <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold">Repayment Period</p>
                            <p id="terms-term" class="text-base font-black text-slate-900 dark:text-slate-100">— months</p>
                        </div>
                        <div class="flex flex-col gap-0.5">
                            <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold">Total Interest</p>
                            <p id="terms-interest" class="text-base font-black text-slate-900 dark:text-slate-100">₵0.00</p>
                        </div>
                ...
                <!-- Terms scroll box -->
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                    <div class="h-60 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-700 p-5 bg-slate-50 dark:bg-slate-800/50 text-xs text-slate-600 dark:text-slate-400 leading-relaxed space-y-3">
                        <p class="font-bold text-slate-900 dark:text-slate-100 text-sm">Loan Agreement Terms &amp; Conditions</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">1. Interest Rates.</span> Personal and Emergency Loans attract an interest rate of <strong>14% per annum</strong>, calculated on a reducing balance basis. Facility Loans attract an interest rate of <strong>16% per annum</strong> on a reducing balance basis. Rates are set by the Fund and may be reviewed periodically with appropriate member notification.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">2. Repayment.</span> Loan repayments shall be deducted at source from the member's monthly salary by the employer and remitted to the Fund not later than the 14th of every month. Failure of the employer to remit deductions does not absolve the member of their repayment obligation.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">3. Eligibility.</span> Applicants must have a minimum of 12 months active membership and contributions in good standing. Outstanding loans must be fully settled before a new loan is granted, unless refinancing is approved by the Fund Administrator.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">4. Default.</span> A loan is considered in default if two or more consecutive monthly instalments are missed. Upon default, the full outstanding balance becomes immediately due, and the Fund reserves the right to recover arrears from the member's contributions or benefit entitlement.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">5. Early Repayment.</span> Members may settle their loan in full or in part at any time without penalty. Partial payments will be applied first to outstanding interest and then to the principal balance.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">6. Loan Insurance.</span> All loans above ₵5,000 are covered by the Fund's group loan protection scheme. In the event of death or total permanent disability, the outstanding loan balance shall be waived. A nominal insurance levy may be applied as disclosed in the Loan Offer Letter.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">7. Disbursement.</span> Approved loan funds will be disbursed to the member's registered bank account on file within 5 working days of full document submission and credit approval.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">8. Disclosure.</span> The Fund may share loan information with relevant regulatory or supervisory bodies as required by law. Members consent to a credit enquiry being conducted as part of the application process.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">9. Governing Law.</span> This agreement is governed by the laws of Ghana and disputes shall be subject to the jurisdiction of the courts of Ghana or the appropriate Alternative Dispute Resolution body as determined by the Fund.</p>

                        <p><span class="font-semibold text-slate-800 dark:text-slate-200">10. Amendments.</span> The Fund reserves the right to amend these terms with 30 days' prior notice to members. Continued participation following such notice constitutes acceptance of the revised terms.</p>
                    </div>
                </div>
                ...
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800 space-y-4">
                    ...
                    @php
                        $declarations = [
                            ['id' => 'chk-read',     'text' => 'I have read and fully understood the loan terms and conditions stated above.'],
                            ['id' => 'chk-accurate',  'text' => 'I confirm that all information and documents provided in this application are true, accurate, and complete.'],
                            ['id' => 'chk-deduction', 'text' => 'I authorise the Fund to deduct monthly loan repayments from my salary/contributions as stated in this agreement.'],
                            ['id' => 'chk-default',   'text' => 'I understand the consequences of default and agree to notify the Fund immediately if I am unable to meet a repayment.'],
                        ];
                    @endphp

                    @foreach ($declarations as $decl)
                        <label for="{{ $decl['id'] }}" class="flex items-start gap-3 cursor-pointer group">
                            <input type="checkbox" id="{{ $decl['id'] }}" class="mt-0.5 size-4 rounded border-slate-300 text-primary focus:ring-primary cursor-pointer" onchange="checkDeclarations()" />
                            <span class="text-sm text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-100 transition-colors leading-relaxed">
                                {{ $decl['text'] }}
                            </span>
                        </label>
                    @endforeach
                ...
                <div class="p-5 sm:p-8 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 items-start">
                        <div class="flex-1 h-28 rounded-lg border-2 border-dashed border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-center">
                            <p class="text-xs text-slate-400 italic">Sign here using a stylus or mouse</p>
                        </div>
                ...
                <div class="p-5 sm:p-8 flex flex-wrap justify-between items-center gap-4 bg-slate-50 dark:bg-slate-900/50">
                    <button onclick="goToStep(3)" class="flex items-center justify-center px-6 h-12 rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors">
                        <span class="material-symbols-outlined mr-2 text-[20px]">arrow_back</span>Back
                    </button>
<button id="submit-btn" disabled form="loan-form"
                        type="submit"
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div class="p-10 sm:p-16 flex flex-col items-center gap-6 text-center">
                    ...
                    <div class="flex flex-col gap-2 max-w-md">
                        ...
                        <p class="text-slate-500 dark:text-slate-400">Your loan application has been received and is under review. You will be notified within <strong>3–5 business days</strong>.</p>
                    </div>
                    <div class="flex items-center gap-3 px-5 py-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                        ...
                        <div class="text-left">
                            ...
                            <p id="ref-number" class="text-sm font-black text-slate-900 dark:text-slate-100">LN-2024-00000</p>
                        </div>
                    </div>
                    <div class="flex flex-wrap gap-3 justify-center mt-2">
                        <a href="#" class="flex items-center gap-2 px-5 h-11 rounded-lg bg-primary text-white font-bold text-sm hover:bg-primary/90 transition-colors">
                            <span class="material-symbols-outlined text-[18px]">download</span>
                            Download Summary
                        </a>
                        <a href="#" class="flex items-center gap-2 px-5 h-11 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
        ...
        <div id="guidance-cards" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div class="flex gap-3 p-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 border-t-4 border-t-accent-gold shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Contact fund support for policy clarifications.</p>
                </div>
            </div>
            <div class="flex gap-3 p-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 border-t-4 border-t-accent-red shadow-sm">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Your financial data is encrypted and protected.</p>
                </div>
            </div>
            <div class="flex gap-3 p-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 border-t-4 border-t-primary shadow-sm">
    ...
    <script>
        ...
        const stepMeta = {
            1: { label: 'Loan Details',    pct: 25  },
            2: { label: 'Calculation',     pct: 50  },
            3: { label: 'Documents',       pct: 75  },
            4: { label: 'Terms',           pct: 100 },
        };
        ...
        function buildAmort(principal, annualRate, months) {
            ...
            if (!principal || !months) {
                tbody.innerHTML = '<tr><td colspan="6" class="px-4 py-8 text-center text-slate-400 text-sm italic">Enter loan details in Step 1 to generate the repayment schedule.</td></tr>';
                return 0;
            }
        ...
        function populateCalc() {
            ...
            const typeLabel = { personal: 'Personal Loan', facility: 'Facility Loan', emergency: 'Emergency Loan' };
