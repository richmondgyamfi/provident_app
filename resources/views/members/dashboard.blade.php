    ...
    <div class="lp-4 sm:p-8 space-y-6 sm:space-y-8">
                ...
                <p class="text-slate-900 dark:text-slate-100 text-3xl font-black leading-tight tracking-tight">Member
                    ...
                    @if ($stats['latest_contribution_year'] && $stats['latest_contribution_month'])
                        <div class="flex items-center gap-1 mt-1">
                            ...
                            <h4>Last Contributions Upload:
                                {{ date('F', mktime(0, 0, 0, $stats['latest_contribution_month'], 1)) }}
                                {{ $stats['latest_contribution_year'] }}</h4>
                        </div>
                    @endif
                </p>
                <p class="text-slate-500 dark:text-slate-400 text-base font-normal">Welcome back,
                    {{ ucfirst(strtolower(auth()->user()->fname)) }}. Here's a snapshot of
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
            <div
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Total
                    Contributions</p>
                <p class="text-2xl font-black text-slate-900 dark:text-slate-100">
                    ₵{{ number_format($stats['personal_contributions'], 2) }}</p>
                ...
                @if ($stats['latest_contribution_year'] && $stats['latest_contribution_month'])
                    <div class="flex items-center gap-1 text-s late-500 text-xs mt-1">
                        <span class="material-symbols-outlined text-xs">calendar_today</span>
                        <span>Last: {{ date('F', mktime(0, 0, 0, $stats['latest_contribution_month'], 1)) }}
                            {{ $stats['latest_contribution_year'] }}</span>
                    </div>
                @endif
            </div>
            <div
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Current Balance
                </p>
                <p class="text-2xl font-black text-slate-900 dark:text-slate-100">
                    ₵{{ number_format($stats['current_balance'], 2) }}</p>
                <div class="flex items-center gap-1 text-emerald-600 text-sm font-bold">
            ...
            <div
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Loan Repayments
                </p>
                <p class="text-2xl font-black text-slate-900 dark:text-slate-100">
                    ₵{{ number_format($stats['total_repayments'], 2) }}</p>
            ...
            <div
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-wider">Active Loan
                    Balance</p>
                <p class="text-2xl font-black text-slate-900 dark:text-slate-100">
                    ₵{{ number_format($stats['active_loans'], 2) }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-8">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div
                            ...
                            <div class="relative z-10">
                                <p class="text-xs font-bold uppercase tracking-widest text-accent-gold mb-1">Financial Tip
                                </p>
                ...
                <div>
                    ...
                    <div
                        class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
                        @forelse($recentActivities as $activity)
                            <div class="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                                <div
                                    class="size-8 rounded-lg bg-{{ $activity['color'] }}-100 dark:bg-{{ $activity['color'] }}-900/30 flex items-center justify-center text-{{ $activity['color'] }}-600 dark:text-{{ $activity['color'] }}-400 flex-shrink-0">
                                    ...
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                        @foreach ($recentContributions as $contribution)
                                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                <td
                                                    class="px-6 py-4 text-sm text-slate-900 dark:text-slate-100 font-medium">
                                                    {{ $contribution->created_at->format('M d, Y') }}</td>
                                                <td class="px-6 py-4">
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-sm text-slate-900 dark:text-slate-100 font-semibold">{{ ucfirst($contribution->contribution_type ?? 'Monthly') }}</span>
                                                        <span
                                                            class="text-[10px] text-slate-400 uppercase font-bold">{{ $contribution->payroll_month ?? 'N/A' }}
                                                            {{ $contribution->payroll_year ?? '' }}</span>
                                                    </div>
                                                </td>
                                                <td
                                                    class="px-6 py-4 text-sm font-bold text-slate-900 dark:text-slate-100 text-right">
                                    ...
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                        @foreach ($recentRepayments as $repayment)
                                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                <td
                                                    class="px-6 py-4 text-sm text-slate-900 dark:text-slate-100 font-medium">
                                                    {{ $repayment->payment_date->format('M d, Y') }}</td>
                                                <td class="px-6 py-4">
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-sm text-slate-900 dark:text-slate-100 font-semibold">{{ $repayment->loan->application_ref }}</span>
                                                        <span
                                                            class="text-[10px] text-slate-400 uppercase font-bold">{{ $repayment->reference ?? 'N/A' }}</span>
                                                    </div>
                                                </td>
                                                <td
                                                    class="px-6 py-4 text-sm font-bold text-slate-900 dark:text-slate-100 text-right">
                                                ...
                                                <td class="px-6 py-4">
                                                    <span
                                                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">{{ ucfirst(str_replace('_', ' ', $repayment->payment_method)) }}</span>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
